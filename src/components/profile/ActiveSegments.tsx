...
const DerivativeTrading: React.FC = () => {
  ...
  return (
    <div 
      className="w-full bg-[#F8F8FB] dark:bg-[#121212] border border-gray-300 dark:border-[#2F2F2F] rounded-md px-4 py-2"
      style={{
        borderRadius: '4px',
        opacity: 1
      }}
    >
      {derivatives.map((item, index) => (
        <div 
          key={item.id} 
          className={`flex items-center justify-between w-full ${index < derivatives.length - 1 ? 'border-b border-gray-200 dark:border-[#2F2F2F]' : ''} py-3`}
          style={{
            opacity: 1
          }}
        >
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              <h3 
                className="font-normal text-gray-900 dark:text-[#EBEEF5] truncate"
                style={{
                  fontSize: '16px',
                  lineHeight: '100%',
                  fontWeight: 400,
            ...
            <p 
              className="text-gray-500 dark:text-[#A0AEC0] mt-3"
              style={{
                fontSize: '12px',
                lineHeight: '100%',
                fontWeight: 400,
